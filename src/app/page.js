...
export default function CatalogPage() {
  ...
  const handleFinalize = async () => {
    ...
    try {
      const response = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ id: userId.trim(), items: cart, total: cartTotal })
      });
      const data = await response.json();
      if (response.ok && data.success) {
        setOrderStatus('success');
        setTimeout(() => {
          setCart([]); setIsCartOpen(false); setIsCheckout(false); setOrderStatus('idle'); setUserId('');
        }, 3000);
      } else { setOrderStatus('error'); }
    } catch (error) { setOrderStatus('error'); }
  };
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans p-4 md:p-8">
      ...
      <header className="max-w-6xl mx-auto mb-8 border-b-2 border-black pb-6 flex justify-between items-end">
        <div className="flex items-center gap-4">
          <div className="relative h-12 w-12 overflow-hidden rounded-full border border-black/10">
            <Image src="/logo.png" alt="Aistesis Logo" fill className="object-cover" priority />
          </div>
          <div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4">
          <div className="flex bg-gray-100 p-1 border-2 border-black">
            {['Todo', 'Bustos', 'Esculturas'].map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 md:px-6 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-black text-white' : 'hover:bg-gray-200 cursor-pointer'}`}>{cat}</button>
            ))}
          </div>
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-black" size={16} />
            <input type="text" placeholder="BUSCAR..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border-2 border-black py-2 pl-10 pr-4 text-[10px] font-black uppercase focus:bg-gray-50 outline-none" />
          </div>
        </div>
      </div>
          ...
          {filteredModels.map((model) => (
            <motion.div layout key={model.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedModel(model)} className="group cursor-pointer border-2 border-black overflow-hidden bg-white">
              <div className="aspect-square relative bg-gray-50 p-6 overflow-hidden">
                <Image src={model.image} alt={model.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-contain p-4 transition-transform duration-500 group-hover:scale-110" />
              </div>
              <div className="p-3 border-t-2 border-black flex justify-between items-center font-black uppercase text-[10px] group-hover:bg-black group-hover:text-white transition-colors">
      ...
      {/* MODAL DETALLE - OPTIMIZADO PARA MÓVILES (SIN SOMBRAS) */}
      <AnimatePresence>
        {selectedModel && (
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-white w-full max-w-4xl border-4 border-black relative flex flex-col md:flex-row overflow-hidden max-h-[95vh]">
              ...
              <div className="h-[35vh] md:h-auto md:flex-1 bg-[#f0f0f0] relative flex items-center justify-center p-4">
                <div className="relative w-full h-full flex items-center justify-center">
                  <Image src={selectedModel.image} alt={selectedModel.name} fill className="object-contain p-2" style={{ transform: `scale(${sizeConfigs[config.size].scale * 0.9})` }} />
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 h-[60%] flex items-center">
                    <motion.div animate={{ height: `${sizeConfigs[config.size].scale * 100}%` }} className="w-[2px] bg-black/20 relative flex items-center">
                      ...
                      <div className="absolute right-4 bg-white border border-black px-1 py-0.5 text-[9px] font-black italic whitespace-nowrap">{sizeConfigs[config.size].label}</div>
                    </motion.div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-5 md:p-10 bg-white border-t-4 md:border-t-0 md:border-l-4 border-black overflow-y-auto">
                ...
                <p className="text-gray-500 text-[9px] md:text-[11px] mb-4 md:mb-6 uppercase font-bold leading-tight">{selectedModel.description}</p>
                
                <div className="space-y-4 md:space-y-6">
                  {/* SELECTOR ESCALA (SIN SOMBRA) */}
                  <div className="space-y-2">
                    <span className="text-[9px] font-black uppercase flex items-center gap-2"><Ruler size={10} /> Escala</span>
                    <div className="grid grid-cols-3 gap-2">
                      {['S', 'M', 'L'].map(s => (
                        <button key={s} onClick={() => setConfig({ ...config, size: s })} className={`py-2 md:py-3 border-2 border-black text-[10px] font-black cursor-pointer transition-all ${config.size === s ? 'bg-black text-white' : 'opacity-40 hover:opacity-100'}`}>{s}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t-2 border-black flex justify-between items-center">
                    ...
                    {/* BOTÓN AÑADIR (SIN SOMBRA EXTRAÑA) */}
                    <button onClick={() => addToCart(selectedModel)} className="bg-black text-white px-6 md:px-10 py-3 md:py-4 font-black uppercase tracking-widest text-[10px] md:text-xs border-2 border-black hover:bg-gray-800 transition-all cursor-pointer">
                      Añadir
                    </button>
                  </div>
                </div>
      ...
      {/* CARRITO Y CHECKOUT (SIN SOMBRAS EN BOTONES) */}
      <AnimatePresence>
        {isCartOpen && (
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-[70] border-l-4 border-black p-6 flex flex-col shadow-2xl overflow-hidden">
              <div className="flex justify-between items-center mb-8 border-b-2 border-black pb-4">
                <div className="flex items-center gap-3">
                  {isCheckout && orderStatus === 'idle' && <button onClick={() => setIsCheckout(false)} className="cursor-pointer hover:scale-110 transition-transform"><ArrowLeft size={20}/></button>}
                    ...
                    {orderStatus === 'success' ? (
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <input type="text" value={userId} onChange={(e) => {setUserId(e.target.value); setOrderStatus('idle');}} placeholder="EJ: FI2026-01" className={`w-full bg-white border-2 p-3 font-black uppercase text-xs focus:bg-gray-50 outline-none ${orderStatus === 'error' ? 'border-red-500' : 'border-black'}`} />
                          {orderStatus === 'error' && <p className="text-[9px] font-black text-red-500 uppercase flex items-center gap-1"><AlertCircle size={10}/> ID no encontrado</p>}
                        </div>
                      </>
                    )}
                  </div>
              ...
              {orderStatus !== 'success' && cart.length > 0 && (
                <div className="border-t-4 border-black pt-6 mt-4">
                  ...
                  <button onClick={!isCheckout ? () => setIsCheckout(true) : handleFinalize} disabled={isCheckout && (!userId || orderStatus === 'loading')} className="w-full bg-black text-white py-5 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-gray-900 transition-colors cursor-pointer flex items-center justify-center gap-3 disabled:opacity-50">
                    {orderStatus === 'loading' ? <Loader2 size={16} className="animate-spin" /> : (!isCheckout ? 'Confirmar Pedido' : 'Finalizar Compra')}
                  </button>
                </div>
              )}
            </motion.div>
